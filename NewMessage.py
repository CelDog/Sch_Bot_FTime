...
def Admin_active(message):
         ...
         bot.send_message(message.chat.id, 'Вы - Админ! Это значит, что вы теперь можете отредактировать расписание уроков.'.format(message.from_user, bot.get_me()), parse_mode='html')   
...
def Any(message):
    if message.chat.type == 'private':
        ...
        #Изменено: 'row_width=2' Так же сделано из твоими кнопками
            markup = types.InlineKeyboardMarkup(row_width=2)
